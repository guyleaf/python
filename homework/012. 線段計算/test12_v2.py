...
def compare(ax1, ax2, bx1, bx2, cx1, cx2): #檢查有無重疊
    ...
    sign4 = 0
#線段a-線段b
    if(bx1 in range(ax1, ax2+1) and bx2 in range(ax1, ax2+1)):
        space1 = bx2 - bx1
    elif(bx1 in range(ax1, ax2+1)):
        sign1 = 1
    if(sign1==1):
        space1= ax2 - bx1
        
#線段a-線段c
    if(cx1 in range(ax1, ax2+1) and cx2 in range(ax1, ax2+1)):
        space2 = cx2 - cx1
    elif(cx1 in range(ax1, ax2+1)):
        sign2 = 1
    if(sign2==1):
        space2= ax2 - cx1
        sign3 = 1
...
#線段a-其他
    if(sign3==1 and cx2 in range(bx1, bx2+1)):
        space3 = cx2 - ax2
    elif(sign3==1):
        space3 = bx2 - ax2
        
#線段b-線段c
    if(cx1 in range(bx1, bx2+1) and cx2 in range(bx1, bx2+1)):
        space4 = cx2 - cx1
    elif(sign2==0 and cx1 in range(bx1, bx2+1)):
        sign4 = 1
    if(sign4==1):
        space4= bx2 - cx1
