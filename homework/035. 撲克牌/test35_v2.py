...
def pair(N): #pair
    F = [N.count(i) for i in N] #取數字個數
    if(F.count(4)==4): return 6 #4個4 代表是鐵支
    elif(F.count(3)==3 and F.count(2)==2): return 5 #3個3及2個2 代表是葫蘆
    elif(F.count(3)==3): return 3 #只有3個3 代表是三條
    elif(F.count(2)==4): return 2 #4個2 代表是two pair
    elif(F.count(2)==2): return 1 #2個2 代表是one pair
    else: return 0 #散牌
...
def Straight(N): #順子
    ...
    if(14 in N and 2 in N): #12 13 14 2 3  也可為順子
        for i in range(5):
            if(N[i] <= 5): N[i] += 13 #12 13 14 2+13 3+13
    total = sum(N) - min(N) * 5 #加總-取最小*5=1+2+3+4+5=10
    if(total == 10): return 1 #判斷有無等於10
    return 0
