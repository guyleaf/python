...
def ip(num): #輸入線段範圍
    ...
    #register = sorted(register) sort可加可不加-有sort可去除計算totalleft，只計算totalright(因為線段預設長度以第一個線段為準)
...
def compare(group, i):
    ...
    for j in range(1, i, 2):
        #判斷第i個線段head有小於第j個線段head 以及 第i個線段tail要大於第j個線段head(小於則直接皆無覆蓋到i)
        if((group[i]<group[j]) and (group[i+1]>group[j])): 
            totalleft = group[j] - group[i] 
            #第j個線段head-第i個線段head 
        ...
        if((group[i]<group[j+1]) and (group[i+1]>group[j+1])):
            ...
            #   ---------__
            # -------------       (_為totalrigjt)
    if((totalright + totalleft)==0): #如果第i個線段皆無覆蓋到i前面的線段，則長度為第i個線段本身長度
